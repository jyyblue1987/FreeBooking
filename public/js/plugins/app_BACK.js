...
!function () {
    ...
    angular.module("app", ["ngRoute", "ngAnimate", "ngSanitize", "ui.bootstrap", "ui.select", "textAngular", "easypiechart", "angular-skycons", "angular-loading-bar", "FBAngular", "app.ctrls", "app.directives", "app.services", "app.ui.ctrls", "app.ui.form.ctrls", "app.ui.form.directives", "app.ui.table.ctrls", "app.chart.ctrls", "app.chart.directives", "app.todo", "app.email.ctrls"]).config(["uiSelectConfig", function (uiSelectConfig) {
        ...
        var routes = ["home", "hotels", "widgets", "ui/buttons", "ui/typography", "ui/panels", "ui/grids", "ui/icons", "ui/misc", "ui/tabs", "ui/toasts", "forms/form-elems", "forms/form-validation", "forms/form-wizard", "charts/charts", "pages/404", "pages/login", "pages/register", "pages/invoice", "pages/search", "pages/projects", "pages/project-detail", "pages/lock-screen", "pages/forget-pass", "pages/timeline", "pages/profile", "tables/data-table", "tables/static-table", "email/inbox", "email/single-mail"];
        routes.forEach(function (route) {
            setRoutes(route)
        }), $routeProvider.when("/", {redirectTo: "/home"}).when("/404", {templateUrl: "views/pages/404.html"}).otherwise({redirectTo: "/404"})
    }])
}();
...
!function () {
    ...
    angular.module("app.chart.ctrls", []).controller("ChartistDemoCtrl", ["$scope", "$interval", function ($scope) {
        ...
        $scope.linedata = {
            ...
            "bar" === data.type && data.element.attr({style: "stroke-width: 40px"})
        }, $scope.piechartdata = {
            series: [20, 30, 25, 25],
            labels: ["20%", "30%", "25%", "25%"]
        }, $scope.donutdata = {
        ...
        }, $scope.epcOpts = {size: 180, lineWidth: 12, lineCap: "square", barColor: "#38b4ee"}, $scope.epcPercent = 80
    }])
}();
...
!function () {
    ...
    angular.module("app.chart.directives", []).directive("chartist", [function () {
        return {
            restrict: "EA",
            transclude: !0,
            scope: {type: "@", opts: "=", data: "=", resOpts: "=", tip: "=", ondraw: "&", oncreated: "&"},
            replace: !0,
            ...
            link: function (scope, el) {
                ...
                scope.$on("chartist.update", function (e, data) {
                    ...
                    if (scope.oncreated({data: data}), scope.tip) {
                        var type;
...
!function () {
    ...
    angular.module("app.email.ctrls", []).controller("EmailCtrl", ["$scope", "$modal", function ($scope, $modal) {
        $scope.labelColors = ["#5974d9", "#19c395", "#fc3644", "#232429", "#f1d44b"], $scope.labels = [{
            ...
            $scope.newlabel && $scope.labels.push({type: $scope.newlabel, color: c}), $scope.newlabel = ""
...
!function () {
    "use strict";
    angular.module("app.ui.form.ctrls", []).factory('myService', function() {
        return {
            submitData: function(url, $http, $data, $scope) {
        ...
        $s.person = {}, $s.people = [{name: "Adam", email: "adam@mail.com"}, {
            name: "Amalie",
        ...
        }, {name: "Natasha", email: "natasha@mail.com"}, {name: "Nicole", email: "nicole@mail.com"}, {
            name: "Adrian",
            email: "adrian@mail.com"
        }], $s.state = {}, $s.timezone = [{tag: 1, name: "Alaska"}, {tag: 1, name: "Hawaii"}, {
            tag: 2,
            name: "California"
        }, {tag: 2, name: "Nevada"}, {tag: 2, name: "Oregon"}, {tag: 2, name: "Washington"}, {
            tag: 3,
            name: "Arizona"
        }, {tag: 3, name: "Colorado"}, {tag: 3, name: "Idaho"}, {tag: 3, name: "Montana"}, {
            tag: 3,
            name: "Nebraska"
        }, {tag: 3, name: "New Mexico"}, {tag: 3, name: "North Dakota"}, {tag: 3, name: "Utah"}, {
            tag: 3,
            name: "Wyoming"
        }, {tag: 4, name: "Alabama"}, {tag: 4, name: "Arkansas"}, {tag: 4, name: "Illinois"}, {
            tag: 4,
            name: "Iowa"
        }, {tag: 4, name: "Kansas"}, {tag: 4, name: "Kentucky"}, {tag: 4, name: "Louisiana"}, {
            tag: 4,
            name: "Minnesota"
        }, {tag: 4, name: "Mississippi"}, {tag: 4, name: "Missouri"}], $s.timezoneFn = function (item) {
            switch (item.tag) {
                case 1:
    ...
    }]).controller("FormWizardCtrl", ["$scope","$http", "myService",  function ($scope, $http, myService) {

            var userData= [];
            var hotelData= [];
            var errors= [];

        $scope.steps = [!0, !1, !1], $scope.stepNext = function (index, action) {
         ...
         if(action == 'user'){
             ...
             hotelData=$scope.hotel;
              ...
              myService.submitData("auth/register",$http, hotelData, $scope);



             //myService.submitData($scope.person.email+"/addHotel",$http, hotelData, $scope);
...
!function () {
    ...
    angular.module("app.todo", []).factory("todoStorage", [function () {
        ...
        var demoTodos = [{title: "Eat healthy, Eat fresh", completed: !1}, {
        ...
        $s.newTodo = "", $s.remainingCount = $filter("filter")(todos, {completed: !1}).length, $s.editedTodo = null, $s.edited = !1, $s.todoshow = "all", $s.$watch("remainingCount == 0", function (newVal) {
            $s.allChecked = newVal
            ...
            switch (filter) {
                case"all":
                    ...
                    break;
                case"active":
                    $s.statusFilter = {completed: !1}
            }
        }, $s.addTodo = function () {
            var newTodo = {title: $s.newTodo.trim(), completed: !1};
...
!function () {
    ...
    angular.module("app.ui.ctrls", []).controller("ToastDemoCtrl", ["$scope", "$interval", function ($scope) {
        $scope.noti = {selected: "Danger"}, $scope.notifications = ["Warning", "Success", "Info", "Danger"], $scope.positionModel = "topRight", $scope.animModel = "fade";
        ...
        $scope.alerts = [{type: "warning", msg: "<strong>Warning:</strong> Backup all your drive."}, {
            type: "danger",
        ...
        $scope.max = 200, $scope.random = function () {
            ...
            for (var types = ["success", "info", "warning", "danger"], i = 0, n = Math.floor(4 * Math.random() + 1); n > i; i++) {
                ...
                $scope.stacked.push({value: Math.floor(30 * Math.random() + 1), type: types[index]})
            }
        }, $scope.randomStacked()
